...
template<typename FPTYPE>
void deepmd::tabulate_fusion_cpu_transpose(
    FPTYPE * out,
    const FPTYPE * table, 
    ...
    const int last_layer_size)
{
  ...
  for (int ii = 0; ii < nloc; ii++) {
    ...
    for (int jj = 0; jj < nnei; jj++) { 
      ...
      const FPTYPE* table0 = &table[table_idx * last_layer_size * 6 + last_layer_size * 0];
      const FPTYPE* table1 = &table[table_idx * last_layer_size * 6 + last_layer_size * 1];
      const FPTYPE* table2 = &table[table_idx * last_layer_size * 6 + last_layer_size * 2];
      const FPTYPE* table3 = &table[table_idx * last_layer_size * 6 + last_layer_size * 3];
      const FPTYPE* table4 = &table[table_idx * last_layer_size * 6 + last_layer_size * 4];
      const FPTYPE* table5 = &table[table_idx * last_layer_size * 6 + last_layer_size * 5];
...
template<typename FPTYPE>
void deepmd::tabulate_fusion_grad_cpu_transpose(
    FPTYPE * dy_dem_x, 
    FPTYPE * dy_dem,
    const FPTYPE * table, 
    const FPTYPE * table_info, 
...
template void deepmd::tabulate_fusion_cpu_transpose<float>(float * out, const float * table, const float * table_info, const float * em_x, const float * em, const int nloc, const int nnei, const int last_layer_size);
template void deepmd::tabulate_fusion_cpu_transpose<double>(double * out, const double * table, const double * table_info, const double * em_x, const double * em, const int nloc, const int nnei, const int last_layer_size);
template void deepmd::tabulate_fusion_grad_cpu_transpose<float> (float * dy_dem_x, float * dy_dem, const float * table, const float * table_info, const float * em_x, const float * em, const float * dy, const int nloc, const int nnei, const int last_layer_size); 
template void deepmd::tabulate_fusion_grad_cpu_transpose<double> (double * dy_dem_x, double * dy_dem, const double * table, const double * table_info, const double * em_x, const double * em, const double * dy, const int nloc, const int nnei, const int last_layer_size);


void deepmd::tabulate_fusion_cpu_transpose_sve(
    double * out,
    const double * table, 
    ...
    const int last_layer_size)
{
  ...
  for (int ii = 0; ii < nloc; ii++) {
    ...
    for (int jj = 0; jj < nnei; jj++) { 
      ...
      const double* table0 = &table[table_idx * last_layer_size * 6 + last_layer_size * 0];
      const double* table1 = &table[table_idx * last_layer_size * 6 + last_layer_size * 1];
      const double* table2 = &table[table_idx * last_layer_size * 6 + last_layer_size * 2];
      const double* table3 = &table[table_idx * last_layer_size * 6 + last_layer_size * 3];
      const double* table4 = &table[table_idx * last_layer_size * 6 + last_layer_size * 4];
      const double* table5 = &table[table_idx * last_layer_size * 6 + last_layer_size * 5];
      ...
      for(int kk = 0; kk < last_layer_size; kk += svcntd() * 2){
        svfloat64_t va0_0 = svld1(ptrue, table0 + kk);
        svfloat64_t va0_1 = svld1(ptrue, table0 + kk + svcntd());
        svfloat64_t va1_0 = svld1(ptrue, table1 + kk);
        svfloat64_t va1_1 = svld1(ptrue, table1 + kk + svcntd());
        svfloat64_t va2_0 = svld1(ptrue, table2 + kk);
        svfloat64_t va2_1 = svld1(ptrue, table2 + kk + svcntd());
        svfloat64_t va3_0 = svld1(ptrue, table3 + kk);
        svfloat64_t va3_1 = svld1(ptrue, table3 + kk + svcntd());
        svfloat64_t va4_0 = svld1(ptrue, table4 + kk);
        svfloat64_t va4_1 = svld1(ptrue, table4 + kk + svcntd());
        svfloat64_t va5_0 = svld1(ptrue, table5 + kk);
        svfloat64_t va5_1 = svld1(ptrue, table5 + kk + svcntd());
      ...
      for (int kk = 0; kk < last_layer_size; kk++) {
        ...
        double var = a0 + a1 * xx + a2 * xx2 + a3 * xx3 + a4 * xx4 + a5 * xx5;
        if (unloop) {
          out0[kk] += (nnei - jj) * var * ll[0];
          out1[kk] += (nnei - jj) * var * ll[1];
          out2[kk] += (nnei - jj) * var * ll[2];
    ...
    const int last_layer_size)
{
  ...
  for (int ii = 0; ii < nloc; ii++) {
    ...
    for (int jj = 0; jj < nnei; jj++) { 
      ...
      const float* table0 = &table[table_idx * last_layer_size * 6 + last_layer_size * 0];
      const float* table1 = &table[table_idx * last_layer_size * 6 + last_layer_size * 1];
      const float* table2 = &table[table_idx * last_layer_size * 6 + last_layer_size * 2];
      const float* table3 = &table[table_idx * last_layer_size * 6 + last_layer_size * 3];
      const float* table4 = &table[table_idx * last_layer_size * 6 + last_layer_size * 4];
      const float* table5 = &table[table_idx * last_layer_size * 6 + last_layer_size * 5];
...
void deepmd::tabulate_fusion_grad_cpu_transpose_sve(
    double * dy_dem_x, 
    double * dy_dem,
    const double * table, 
    const double * table_info, 
    ...
    for (int ii = 0; ii < nloc; ii++) {
        ...
        for (int jj = 0; jj < nnei; jj++) {
            ...
            for (int kk = 0; kk < last_layer_size; kk++) {
                ...
                double res = a0 + a1 * xx + a2 * xx2 + a3 * xx3 + a4 * xx4 + a5 * xx5;
                // double grad_tmp = (a1 + (2 * a2 + (3 * a3 + (4 * a4 + 5 * a5 * xx) * xx) * xx) * xx) * dot(ll, rr);
                double grad_tmp = (a1 + 2 * a2 * xx + 3 * a3 * xx2 + 4 * a4 * xx3 + 5 * a5 *xx4 ) * dot(ll, rr);
                if (unloop) {
                    grad += grad_tmp * (nnei - jj);
                    dy_dem_tmp[0] += res * rr[0] * (nnei - jj);
                    dy_dem_tmp[1] += res * rr[1] * (nnei - jj);
                    dy_dem_tmp[2] += res * rr[2] * (nnei - jj);
                    dy_dem_tmp[3] += res * rr[3] * (nnei - jj);
                }
                else {
                    grad += grad_tmp;
                    dy_dem_tmp[0] += res * rr[0];
                    dy_dem_tmp[1] += res * rr[1];
                    dy_dem_tmp[2] += res * rr[2];
                    dy_dem_tmp[3] += res * rr[3];
                }
            }
#endif /* __ARM_FEATURE_SVE */
            dy_dem_x[ii * nnei + jj] = grad;
